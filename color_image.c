...
static void allocChannels(ColorImage *img)
{
    ...
    img->A = img->hasAlpha ? (int **)malloc(img->height * sizeof(int *)) : NULL;
...
static ColorImage *load32BitBMP(const char *filename)
{
    ...
    if (!fp) { printf("Error opening BMP file: %s\n", filename); return NULL; }
    ...
    img->height    = isBottomUp ? ih.biHeight : -ih.biHeight;
    ...
    for (int i = 0; i < img->height; i++) {
        int row = isBottomUp ? (img->height - 1 - i) : i;
        ...
        if (rowPadding > 0) fread(padBuf, 1, rowPadding, fp);
    }
    fclose(fp);
    return img;
}
...
static void save32BitBMP(const char *filename, ColorImage *img)
{
    ...
    if (!fp) { printf("Error creating BMP file: %s\n", filename); return; }
...
static ColorImage *loadPPMImage(const char *filename)
{
    ...
    if (strcmp(magic, "P3") != 0 && strcmp(magic, "P6") != 0 &&
        strcmp(magic, "P7") != 0) {
        printf("Unsupported format: %s (supported: P3, P6, P7)\n", magic);
        fclose(fp); free(img); return NULL;
    }
